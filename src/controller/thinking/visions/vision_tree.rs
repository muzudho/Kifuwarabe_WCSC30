...
pub fn insert_rakkansuji(universe: &mut Universe) {
    for phase43 in SN_ARRAY.iter() {
        let opponent_phase = hanten_sn(phase43);

        {
            // 相手の　らいおん　の位置を覚える
            universe
                .get_search_part_mut()
                .memory_opponent_king(phase43, &opponent_phase);
        }
        {
            ...
            for kms_dst in KMS_ARRAY.iter() {
                let ps_dst = PieceVo::from_piece(Piece::from_phase_piece_type(&phase43, &kms_dst));
                for x in SUJI_1..SUJI_10 {
                    ...
                    for y in DAN_1..DAN_10 {
                        ...
                        for sq_src in mv_src_hashset.iter() {
                            ...
                            .to_hash();
                            &universe.get_search_part_mut().vision_tree_by_phase
                                [sn_to_num(phase43)]
                            .ss_tume_hashset
                            .insert(hash_ss);
                        }
                        ...
                        for kms_da in da_kms_hashset.iter() {
                            let km_da = sn_kms_to_km( &phase43, &kms_da );
                            let hash_ss = Sasite{
                                src:SS_SRC_DA,
                                dst:ms_dst,
                                pro:false,
                            ...
                            &universe.vision_tree_by_sn[phase43].ss_tume_hashset.insert( hash_ss );
                        }
                        */
                    }
                }
