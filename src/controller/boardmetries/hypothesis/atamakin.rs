...
pub fn is_s(universe: &Universe, speed_of_light: &MLSpeedOfLightVo) -> bool {
    // 相手玉の位置
    ...
    let ps = speed_of_light.ml_piece_struct_master_vo.get_piece_vo(
        universe
            .get_search_part()
            .get_current_position()
            .get_piece_by_square(&sq_south_r),
    );
    let jiai_km = universe.get_person_by_piece_vo(&ps);
