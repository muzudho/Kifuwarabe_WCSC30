        ...
        match *p_kmdir {
            ...
            E(b) => {
                if b {
                    ...
                    // 東
                    make_no_promotion_source_by_piece_to_east(
                        square_dst,
                        ps_dst,
                        current_position,
                        speed_of_light,
...
/// 成る前を含めない、東
fn make_no_promotion_source_by_piece_to_east<F1>(
    square_dst: &Square,
    ps_dst: &GPPieceStructVo,
    current_position: &SPPositionDto,
    speed_of_light: &MLSpeedOfLightVo,
...
/// 移動先升、 東
fn make_destination_to_east<S: BuildHasher>(
    sq_dst_ps_src: &GPSquareAndPieceStructVo,
    current_position: &SPPositionDto,
    speed_of_light: &MLSpeedOfLightVo,
    result: &mut HashSet<Square, S>,
) {
    SquareScanner::next_to_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_north_east<S: BuildHasher>(
    ...
    SquareScanner::for_each_north_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_north_east<S: BuildHasher>(
    ...
    SquareScanner::next_to_north_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_north_north_east<S: BuildHasher>(
    ...
    SquareScanner::next_to_north_north_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_north<S: BuildHasher>(
    ...
    SquareScanner::for_each_north(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_north<S: BuildHasher>(
    ...
    SquareScanner::next_to_north(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_north_north_west<S: BuildHasher>(
    ...
    SquareScanner::next_to_north_north_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_north_west<S: BuildHasher>(
    ...
    SquareScanner::for_each_north_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_north_west<S: BuildHasher>(
    ...
    SquareScanner::next_to_north_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_west<S: BuildHasher>(
    ...
    SquareScanner::next_to_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_south_west<S: BuildHasher>(
    ...
    SquareScanner::for_each_south_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_south_west<S: BuildHasher>(
    ...
    SquareScanner::next_to_south_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_south_south_west<S: BuildHasher>(
    ...
    SquareScanner::next_to_south_south_west(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_south<S: BuildHasher>(
    ...
    SquareScanner::for_each_south(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_south<S: BuildHasher>(
    ...
    SquareScanner::next_to_south(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_south_south_east<S: BuildHasher>(
    ...
    SquareScanner::next_to_south_south_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_sliding_to_south_east<S: BuildHasher>(
    ...
    SquareScanner::for_each_south_east(&sq_dst_ps_src.square, &mut |next_square| {
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
        }
...
fn make_destination_to_south_east<S: BuildHasher>(
    ...
    SquareScanner::next_to_south_east(&sq_dst_ps_src.square, &mut |next_square| {
        // 自駒でなければ進める。
        let dst_phase = current_position.get_phase_by_sq(&next_square, speed_of_light);
        if &dst_phase != &sq_dst_ps_src.piece_struct.phase() {
            result.insert(next_square);
...
fn make_no_promotion_source_by_phase_sliding_to_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_north_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_north_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_north_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_north_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_north_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_sliding_to_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_north_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_sliding_to_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_south_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_south_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_south_west(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_south_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_south_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_sliding_to_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_south_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_no_promotion_source_by_phase_to_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_east(&dst_sq_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_north_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_north_north_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_north_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_north<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_north(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_north<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
...
fn make_before_promotion_source_by_phase_to_north_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_north_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_north_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_north_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_north_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_south_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_south_south_west<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_south_west(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_south<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_south(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_south<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_south_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_south_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_sliding_to_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::for_each_south_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
...
fn make_before_promotion_source_by_phase_to_south_east<F1>(
    ...
    F1: FnMut(Square),
{
    SquareScanner::next_to_south_east(&dst_sq_and_demoted_piece.square, &mut |next_square| {
        let exists_piece = current_position.get_piece_by_square(&next_square);
        if *exists_piece == dst_sq_and_demoted_piece.piece {
            gets_square(next_square);
        }
