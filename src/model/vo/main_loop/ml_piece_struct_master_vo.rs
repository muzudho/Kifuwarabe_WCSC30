...
impl MLPieceStructMasterVo {
    /// 駒の属性を参照するぜ☆（＾～＾）
    pub fn get_piece_vo(&self, piece: &GPPieceVo) -> &PieceStructVo {
        // 列挙型を配列のインデックスとして使用☆（＾～＾）
    ...
    pub fn get_piece_vo_by_phase_and_piece_type(
        ...
        match *phase {
            Phase::First => match piece_type {
                King => self.get_piece_vo(&King1),
                Rook => self.get_piece_vo(&Rook1),
                Bishop => self.get_piece_vo(&Bishop1),
                Gold => self.get_piece_vo(&Gold1),
                Silver => self.get_piece_vo(&Silver1),
                Knight => self.get_piece_vo(&Knight1),
                Lance => self.get_piece_vo(&Lance1),
                Pawn => self.get_piece_vo(&Pawn1),
                Dragon => self.get_piece_vo(&Dragon1),
                Horse => self.get_piece_vo(&Horse1),
                PromotedSilver => self.get_piece_vo(&PromotedSilver1),
                PromotedKnight => self.get_piece_vo(&PromotedKnight1),
                PromotedLance => self.get_piece_vo(&PromotedLance1),
                PromotedPawn => self.get_piece_vo(&PromotedPawn1),
                _ => self.get_piece_vo(&GPPieceVo::Owari),
            },
            Phase::Second => match piece_type {
                King => self.get_piece_vo(&King2),
                Rook => self.get_piece_vo(&Rook2),
                Bishop => self.get_piece_vo(&Bishop2),
                Gold => self.get_piece_vo(&Gold2),
                Silver => self.get_piece_vo(&Silver2),
                Knight => self.get_piece_vo(&Knight2),
                Lance => self.get_piece_vo(&Lance2),
                Pawn => self.get_piece_vo(&Pawn2),
                Dragon => self.get_piece_vo(&Dragon2),
                Horse => self.get_piece_vo(&Horse2),
                PromotedSilver => self.get_piece_vo(&PromotedSilver2),
                PromotedKnight => self.get_piece_vo(&PromotedKnight2),
                PromotedLance => self.get_piece_vo(&PromotedLance2),
                PromotedPawn => self.get_piece_vo(&PromotedPawn2),
                _ => self.get_piece_vo(&GPPieceVo::Owari),
            },
            Phase::None => self.get_piece_vo(&GPPieceVo::Owari),
        }
    }
}
