...
pub struct SPDto {
    ...
    /// 取った駒
    pub captured_piece_history: [GPPieceVo; TEME_LN],
    /// 棋譜
...
impl SPDto {
    ...
    pub fn undo_move(
        ...
        let piece186 = if move2.src.to_umasu() == SS_SRC_DA {
            // 打なら
            let piece679 = GPPieceVo::from_phase_piece_type(phase, move2.drop);
            // 自分の持ち駒を増やす
            //let mg = km_to_mg(km);
            //self.add_hand(mg,1);
    ...
    pub fn do_move(
...

        {
            ...
            let piece144 = if move1.src.to_umasu() == SS_SRC_DA {
                // 打なら
                // 自分の持ち駒を減らす
                let piece734 = GPPieceVo::from_phase_piece_type(&phase, move1.drop);
                self.current_position
                ...
                self.current_position
                    .set_piece_by_square(&move1.src, &GPPieceVo::NonePiece);

                piece152
            };

            // 移動先升に駒があるかどうか
            cap = if let GPPieceVo::NonePiece =
                self.current_position.get_piece_by_square(&move1.dst)
            {
                GPPieceVo::NonePiece
            } else {
