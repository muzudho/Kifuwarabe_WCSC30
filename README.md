...
* `DepthNotToGiveUp` - 思考時間の上限を無視して考え続ける深さ。MaxThinkSec より強い。玉を取られたら負けと判定しているので、王手放置漏れを回避するために最低５手（ディスカバード・アタックを検知するのに５手）は探索する必要があるぜ☆（＾～＾）そして６手は読まないと王手のために角をすぐ切るが、５手を読み切れるほどの探索速度は無いぜ☆（＾～＾）
